    ...
    if Result is True:
        ...
        class CurveTrim_S(component):
            ...
            def RegisterInputParams(self, pManager):
                ...
                self.SetUpParam(p, "Type", "T", "曲线延伸类型，0=Line，1=Arc，2=Smooth")
                LINE_TYPE = 0
            ...
            def trim_extension_curve(self, curve):
                sim_curve = curve.Simplify(rg.CurveSimplifyOptions.All, sc.doc.ModelAbsoluteTolerance, sc.doc.ModelAngleToleranceDegrees)
                curve = sim_curve if sim_curve else curve
                after_shear = eval('curve.{}(rg.CurveEnd.Start,self.first_line_length)'.format(self.first_factor[0])) if self.first_type_of_line is None else eval('curve.{}(rg.CurveEnd.Start,self.first_line_length,{})'.format(self.first_factor[0], self.first_type_of_line))
                result_line = eval('after_shear.{}(rg.CurveEnd.End,self.second_line_length)'.format(self.second_factor[0])) if self.second_type_of_line is None else eval('after_shear.{}(rg.CurveEnd.End,self.second_line_length,{})'.format(self.second_factor[0], self.second_type_of_line))
                return result_line
        ...
        class OffsetBySerial(component):
            def __new__(cls):
                instance = Grasshopper.Kernel.GH_Component.__new__(cls,
                                                                   ...
                                                                   """多折线按指定序号进行偏移""", "Scavenger", "B-Curve")
                return instance
                    ...
                    for c_index, c_item in enumerate(line_list):
                        if c_index in sub_index_list:
                            ...
                            if sub_dis_list[count - 1]:
                                new_item = c_item.Offset(curve_planar, sub_dis_list[count - 1], sc.doc.ModelAbsoluteTolerance, rg.CurveOffsetCornerStyle.
                                None)[0]
                            else:
                                new_item = c_item
        ...
        class Filp_Curve(component):
            def __new__(cls):
                instance = Grasshopper.Kernel.GH_Component.__new__(cls,
                                                                   "RPP_Filp Curve", "W33 Curve", """统一该曲线的方向。
                           传入曲线则所有曲线跟曲线方向保持一致
                           传入真值则为一组数据中曲线中心点最高的曲线为统一方向
                           反之中心点最低的曲线""", "Scavenger", "B-Curve")
                return instance
        ...
        class PtsCurveRelationship(component):
            def __new__(cls):
                instance = Grasshopper.Kernel.GH_Component.__new__(cls,
                                                                   "RPP_PtsCurveRelationship", "W4", """确定所选取的点与封闭曲线之间的关系""", "Scavenger", "B-Curve")
                return instance
        ...
        class CurveVectorParallel(component):
            def __new__(cls):
                instance = Grasshopper.Kernel.GH_Component.__new__(cls,
                                                                   "RPP_CurveVectorParallel", "W24", """判断线是否与置顶的向量平行（包含反向平行）""", "Scavenger", "B-Curve")
                return instance
        ...
        class TweenCurves(component):
            def __new__(cls):
                instance = Grasshopper.Kernel.GH_Component.__new__(cls,
                                                                   "RPP_TweenCurves", "W11", """在两曲线中创建多条中间线""", "Scavenger", "B-Curve")
                return instance
        ...
        class PlaneTrimCurve(component):
            def __new__(cls):
                instance = Grasshopper.Kernel.GH_Component.__new__(cls,
                                                                   "RPP_PlaneTrimCurve", "W14", """移除平面一侧的曲线部分""", "Scavenger", "B-Curve")
                return instance
        ...
        class UnifyCurve(component):
            def __new__(cls):
                instance = Grasshopper.Kernel.GH_Component.__new__(cls,
                                                                   "RPP_UnifyCurve", "W34", """统一曲线方向为逆时针""", "Scavenger", "B-Curve")
                return instance
        ...
        class RemoveOverlapCurve(component):
            def __new__(cls):
                instance = Grasshopper.Kernel.GH_Component.__new__(cls,
                                                                   "RPP_RemoveOverlapCurve", "W12", """删除重合的线段和曲线，可调节公差将相似重合的曲线删除""", "Scavenger", "B-Curve")
                return instance
            ...
            def RunScript(self, curves, tol):
                try:
                    ...
                    if len(re_mes) > 0:
                        ...
                        curve_list = self.unify_curve_direction(curves[0], curves)
        ...
        class IntersectionBreak(component):
            def __new__(cls):
                instance = Grasshopper.Kernel.GH_Component.__new__(cls,
                                                                   "RPP_IntersectionBreak", "W23", """曲线相交位置打断曲线""", "Scavenger", "B-Curve")
                return instance
        ...
        class ExtendTargetCurve(component):
            def __new__(cls):
                instance = Grasshopper.Kernel.GH_Component.__new__(cls,
                                                                   "RPP_ExtendTargetCurve", "W21", """将原始曲线延伸至目标曲线""", "Scavenger", "B-Curve")
                return instance
        ...
        class CurveDirGroupByGeo(component):
            def __new__(cls):
                instance = Grasshopper.Kernel.GH_Component.__new__(cls,
                                                                   "RPP_CurveDirGroupByGeo", "W22", """将曲线最接近的物体的一端作为曲线的起点或终点""", "Scavenger", "B-Curve")
                return instance
        ...
        class BoundingRectangle(component):
            def __new__(cls):
                instance = Grasshopper.Kernel.GH_Component.__new__(cls,
                                                                   "RPP-BoundingRectangle", "W41", """为一组几何物体创建边界矩形""", "Scavenger", "B-Curve")
                return instance
            ...
            def get_boxegde(self, obj):
                if type(obj) is rg.Rectangle3d:
                    obj = obj.ToNurbsCurve()
                pts_list = [[_] if type(_) is rg.Point3d else rg.Box(self.ref_plane, _.ToNurbsCurve()).GetCorners() for _ in obj]
